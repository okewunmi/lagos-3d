...
// Ground texture-getter per environment type — the base terrain
// under and around each chapter's action. Real generated textures,
// not flat fill colors.
const GROUND_TEXTURE = {
  water: getGrassTexture,      // lagoon bed edge, mostly hidden under the water plane
  forest: getGrassTexture,
  road: getCourtyardTexture,   // surrounding land either side of the clay path itself
  courtyard: getCourtyardTexture,
};
...
function Tree({ x, z, scale = 1, lean = 0, hue = 0 }) {
  const foliage = useMemo(() => getFoliageTexture(), []);
  const canopyColor = useMemo(() => {
    const c = new THREE.Color('#356b42');
    c.offsetHSL(hue, 0, 0);
    return c;
  }, [hue]);
  return (
    <group position={[x, 0, z]} scale={scale} rotation={[0, 0, lean]}>
      <mesh position={[0, 0.9, 0]} castShadow>
        ...
        <meshStandardMaterial color="#4a3624" roughness={1} />
      </mesh>
      <mesh position={[0, 2.15, 0]} castShadow>
        <coneGeometry args={[0.95, 1.7, 8]} />
        <meshStandardMaterial map={foliage} color={canopyColor} roughness={0.9} />
      </mesh>
      <mesh position={[0, 3.0, 0]} castShadow>
        <coneGeometry args={[0.68, 1.25, 8]} />
        <meshStandardMaterial map={foliage} color={canopyColor} roughness={0.9} />
      </mesh>
    </group>
  );
...
function WaterPlane({ zMid, length }) {
  const tex = useMemo(() => {
    const t = getWaterTexture().clone();
    t.needsUpdate = true;
    t.repeat.set(6, Math.max(2, Math.round(length / 8)));
    return t;
  }, [length]);
  return (
    <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.03, zMid]}>
      <planeGeometry args={[70, length + 2]} />
      <meshStandardMaterial map={tex} transparent opacity={0.88} roughness={0.2} metalness={0.2} />
    </mesh>
  );
}
...
function RoadStrip({ zMid, length }) {
  const tex = useMemo(() => {
    const t = getDirtRoadTexture().clone();
    t.needsUpdate = true;
    t.repeat.set(1, Math.max(2, Math.round(length / 6)));
    return t;
  }, [length]);
  return (
    <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.01, zMid]}>
...
function Scatter({ segment }) {
  ...
  const items = useMemo(() => {
    const out = [];
    const count = segment.chapter.environment === 'courtyard' ? 0 : 6;
    for (let i = 0; i < count; i++) {
      const z = segment.zLow + rand() * segment.length;
      const side = rand() < 0.5 ? -1 : 1;
      const x = side * (9 + rand() * 8);
      ...
      const hue = (rand() - 0.5) * 0.06;
      out.push({ x, z, scale, lean, hue });
    }
    return out;
  }, [segment, rand]);
...
function GroundPatch({ seg }) {
  const baseTex = useMemo(() => {
    const getTex = GROUND_TEXTURE[seg.chapter.environment] || getGrassTexture;
    const t = getTex().clone();
    t.needsUpdate = true;
    t.repeat.set(10, Math.max(3, Math.round(seg.length / 3)));
    return t;
  }, [seg]);

  return (
    <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, -0.05, seg.zMid]} receiveShadow>
      <planeGeometry args={[80, seg.length + 0.5]} />
      <meshStandardMaterial map={baseTex} roughness={1} />
    </mesh>
  );
}
